...
BT_DECO_CONDITION_UNIT_QUORUM::BT_DECO_CONDITION_UNIT_QUORUM(std::string name, BWAPI::UnitType unitType, BT_NODE* parent, CONDITION_CBK conditionCBK, int quorum, bool triggerOnlyOnce)
    : ConditionCBK(conditionCBK), BT_DECORATOR(name, parent), m_unitType(unitType), m_quorum(quorum), m_triggerOnlyOnce(triggerOnlyOnce)
{
}
...
{
	if(triggerOnlyOnce && m_hasAlreadyTriggered) return true;
